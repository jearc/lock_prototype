...
#define TRANSITIONS_ARR_SIZE 1024 * 1024 * 32
volatile int lock_acq_ids [TRANSITIONS_ARR_SIZE];
int lengths [TRANSITIONS_ARR_SIZE];
volatile int lock_acq_ids_entry = 0;
...
void *test(void *data)
{
    ...
    while (stop == 0) {
        ...
        if (lock_acq_ids_entry < TRANSITIONS_ARR_SIZE)
            lock_acq_ids[lock_acq_ids_entry++] = d->id;
...
int main(int argc, char **argv)
{
    ...
    for (i  = 0; i < TRANSITIONS_ARR_SIZE; ++i) {
        lock_acq_ids[i] = TRANSITIONS_ARR_SIZE;
    }
    lock_acq_ids_entry = 0;
...
//#endif
    ...
    qsort(lengths, transitions, sizeof(*lengths), comp);
    for(a = 0; a <= transitions; ++a) {
        suml += lengths[a];
    }
    printf("\nSamples: %u\n", lock_acq_ids_entry);
    printf("Median: %u\n", lengths[transitions/2]);
    printf("Mean: %f\n", suml * 1.0 / transitions);

    printf("Core change %f, Core stay %f\n", transitions * 100.0 / lock_acq_ids_entry, 
            (lock_acq_ids_entry - transitions) * 100.0 / lock_acq_ids_entry);
